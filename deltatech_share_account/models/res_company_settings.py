...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        res.update(company_share_account=not bool(account_rule.active),)
        account_tax_rule = self.env.ref("account.tax_comp_rule")
        res.update(company_share_account_tax=not bool(account_tax_rule.active),)
                ...
                if company:
                    ...
                    for property in todo_list:
                        properties = PropertyObj.search(
                            [("name", "=", property), ("company_id", "=", self.company_id.id)]
                        )
                        if not properties:
                            properties = PropertyObj.search([("name", "=", property), ("company_id", "=", company.id)])
                            if properties:
