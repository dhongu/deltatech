...
class service_order(models.Model):
    ...
    agreement_id = fields.Many2one('service.agreement', string='Contract Services' , readonly=True,  states={'draft': [('readonly', False)]})
    address_id = fields.Many2one('res.partner', string='Location', readonly=True,  states={'draft': [('readonly', False)]})
    ...
    def create(self,   vals ):  
        if ('name' not in vals) or (vals.get('name') in ('/', False)):
            sequence_order = self.env.ref('deltatech_service_rent.sequence_order')
            if sequence_order:
    ...
    def onchange_equipment_id(self):
        if self.equipment_id:
            ...
            if self.equipment_id.agreement_id:
                self.agreement_id = self.equipment_id.agreement_id
                self.partner_id = self.equipment_id.agreement_id.partner_id
    ...
    def onchange_notification_id(self):
        if self.notification_id:
            ...
            self.address_id = self.notification_id.address_id
            self.agreement_id = self.notification_id.agreement_id
            self.partner_id = self.notification_id.partner_id
