...
class service_efficiency_report(models.Model):
    ...
    def _from(self):
        from_str = super(service_efficiency_report,self)._from() + ' INNER JOIN service_equipment as equi ON  sp.equipment_id = equi.id ' 
        return from_str
    
    def _group_by(self):       
        group_by_str =  super(service_efficiency_report,self)._group_by() + ', equipment_id, agreement_id, pt.uom_shelf_life'
        return group_by_str
    ...
    def get_usage(self,begin_date,end_date, equipment_id, uom_usage, product_id  ):
        ...
        if uom_usage and equipment_id:   
            meter = self.env['service.meter'].search([('equipment_id','=',equipment_id),('uom','=',uom_usage)]) 
            if meter:
