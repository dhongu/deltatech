...
class deltatech_expenses_deduction(models.Model):
    ...
    date_expense = fields.Date(
        string="Expense Date", readonly=True, states={"draft": [("readonly", False)]}, index=True
    )
    ...
    amount_vouchers = fields.Monetary(
        string="Vouchers Amount", digits=dp.get_precision("Account"), compute="_compute_amount"
    )
    advance = fields.Monetary(
        string="Advance", digits=dp.get_precision("Account"), readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    journal_payment_id = fields.Many2one(
        "account.journal", string="Journal Expenses Deduction", required=True
            ...
            for voucher in expenses.expenses_line_ids:
                if not voucher.paid:
                    ...
                    for line in voucher.move_ids:  # de regula este o singura linie
                        if (
                            line.state == "valid"
                            and line.account_id.voucher_type == "payable"
                    ...
                    payment = payments.with_context(context).create(
                        {
                            "journal_id": expenses.journal_payment_id.id,
                            "account_id": expenses.journal_payment_id.default_credit_account_id.id,
                            "type": "payment",
