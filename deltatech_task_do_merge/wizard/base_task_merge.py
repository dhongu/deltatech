...
class MergeTaskAutomatic(models.TransientModel):
    ...
    task_ids = fields.Many2many("project.task", "task_rel", "task_merge_id", "task_id", string="Tasks to merge")
    ...
    def _merge(self, task_ids, dst_task=None):
        ...
        if len(task_ids) > 10:
            raise UserError(
                _(
                    "For safety reasons, you cannot merge more than 10 task together.\n"
        ...
        dst_task.message_post(
            body="%s %s"
            % (_("Merged with the following tasks:"), ", ".join("%s  (ID %s)" % (p.name, p.id) for p in src_tasks))
        )
    ...
    def _action_next_screen(self):
        ...
        if self.line_ids:
            ...
            values.update({"current_line_id": False, "task_ids": [], "state": "finished"})
    ...
    def _process_query(self, query):
        ...
        for min_id, aggr_ids in self._cr.fetchall():
            ...
            self.env["base.task.merge.line"].create({"wizard_id": self.id, "min_id": min_id, "aggr_ids": tasks.ids})
            counter += 1
