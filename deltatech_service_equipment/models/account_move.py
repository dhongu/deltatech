...
class AccountInvoice(models.Model):
    ...
    def generate_excel_meters_report(self):
        ...
        for line in lines:
            ...
            worksheet.write(crt_row, 5, '{0:,.0f}'.format(line['min']))
            worksheet.write(crt_row, 6, '{0:,.0f}'.format(line['max']))
            worksheet.write(crt_row, 7, '{0:,.0f}'.format(line['max'] - line['min']))
            crt_row += 1
            total_readings += (int(line['max']) - int(line['min']))
        crt_row += 1
        worksheet.write(crt_row, 6, 'Total: ', style)
        worksheet.write(crt_row, 7, '{0:,.0f}'.format(total_readings), style)
