...
class service_meter(models.Model):
    ...
    total_counter_value = fields.Float(
        string="Total Counter Value", digits=dp.get_precision("Meter Value"), compute="_compute_last_meter_reading"
    )
    estimated_value = fields.Float(
        string="Estimated Value", digits=dp.get_precision("Meter Value"), compute="_compute_estimated_value"
    )
...
class service_meter_reading(models.Model):
    ...
    counter_value = fields.Float(string="Counter Value", digits=dp.get_precision("Meter Value"), group_operator="max")
    ...
    def _compute_difference(self):
        self.difference = self.counter_value - self.previous_counter_value
        next = self.env["service.meter.reading"].search(
            [("meter_id", "=", self.meter_id.id), ("date", ">", self.date)], limit=1, order="date, id"
        )
        if next and next.previous_counter_value != self.counter_value:
            next.write(
                {"previous_counter_value": self.counter_value, "difference": (next.counter_value - self.counter_value)}
            )
