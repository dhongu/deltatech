...
class FieldsInet(fields._String):
    type = "char"
    column_type = ("inet", "inet")


class FieldsTimestamptz(fields.Datetime):
    column_type = ("timestamptz", "timestamptz")


class FieldsInt8(fields.Integer):
...
# Groups
# ----------------------------------------------------------
class RadiusGroups(models.Model):
    _name = "radius.groups"
    _description = "Groups"
...
class RadiusNas(models.Model):
    ...
    type = fields.Selection(
        [("cisco", "cisco"), ("portslave", "portslave"), ("other", "other")],
        "Nas Type",
        size=32,
        default="other",
        required=True,
...
# Radacct
# ----------------------------------------------------------
class RadiusRadacct(models.Model):
    _name = "radius.radacct"
    _description = "Radacct"

    name = fields.Char("Name", size=64)  # new
    radacctid = FieldsInt8(related="id", string="Rad Acct Id", store=True)  # care trebuie sa fie de fapt ID
    ...
    nasipaddress = FieldsInet("Nas IP Address")
    nasportid = fields.Char("Nas Port Id", size=64)
    nasporttype = fields.Char("Nas port type", size=64)
    acctstarttime = FieldsTimestamptz("Acct Start time")
    acctupdatetime = FieldsTimestamptz("Acct Update time")
    acctstoptime = FieldsTimestamptz("Acct stop time")
    acctinterval = FieldsInt8(string="Acct Interval")
    acctsessiontime = FieldsInt8("Acct session time")
    ...
    acctinputoctets = FieldsInt8("Acct input octets")
    acctoutputoctets = FieldsInt8("Acct output octets")
    ...
    framedipaddress = FieldsInet("Framed IP Address")
...
# Radcheck
# ----------------------------------------------------------
class RadiusRadcheck(models.Model):
    ...
    groupname = fields.Selection(
        _get_groupname,
        string="Contract Group Name",
        default="",
        readonly=False,
    ...
    state = fields.Selection(
        [("connected", "Connected"), ("disconnected", "Disconnected")], compute="_compute_groupname"
    )
...
# Radreply
# ----------------------------------------------------------
class RadiusRadreply(models.Model):
...
# Radgroupcheck
# ----------------------------------------------------------
class RadiusRadgroupcheck(models.Model):
...
# Radgroupreply
# ----------------------------------------------------------
class RadiusRadgroupreply(models.Model):
...
# Radusergroup
# ----------------------------------------------------------
class RadiusRadusergroup(models.Model):
...
# Radpostauth
# ----------------------------------------------------------
class RadiusRadpostauth(models.Model):
    _name = "radius.radpostauth"
    _description = "radpostauth"

    _fields = {"pass": fields.Char(string="Password", size=64)}
...
class RadiusRadippool(models.Model):
    ...
    framedipaddress = FieldsInet(string="Framed IP address", index=1, required=True)
