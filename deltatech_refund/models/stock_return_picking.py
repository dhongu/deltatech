...
class stock_return_picking(models.TransientModel):
    ...
    def _create_returns(self): 
        ...
        if self.make_new_picking: 
            ...
            msg = _('%s was generated to transfer products from %s they have been refunded by %s ') % (self.get_link(pick_backorder), 
                                                                                                     self.get_link(pick),
            ...
            msg = _('It was not generated a new picking list for transfer products from %s they have been refunded by %s ') % (self.get_link(pick),
                                                                                                     self.get_link(pick_return) )
            pick.message_post(body=msg)               
        if self.do_transfer:
