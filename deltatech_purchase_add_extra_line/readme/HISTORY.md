...
- a price the standard writes on the extra line itself (a vendor price recomputation) is kept as well: `purchase.order.line` has no `technical_price_unit` in this version, so it cannot be told apart from a price typed in by the user
