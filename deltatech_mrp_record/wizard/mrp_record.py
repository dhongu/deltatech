...
class MrpRecord(models.TransientModel):
    ...
    production_id = fields.Many2one(
        "mrp.production", string="Production Order", domain=[("state", "in", ["planned", "progress"])]
    )

    procurement_group_id = fields.Many2one(
        "procurement.group", "Procurement Group", related="production_id.procurement_group_id"
    )
    ...
    qty_ready_prod = fields.Float("Quantity Ready for Production", readonly=True, related="workorder_id.qty_ready_prod")
    ...
    def onchange_workorder_id(self):
        ...
        self.date_start = (
            self.workorder_id.date_planned_start or self.production_id.date_planned_start or fields.Datetime.now()
        )
        self.date_end = (
            self.workorder_id.date_planned_finished or self.production_id.date_planned_finished or fields.Datetime.now()
        )
        ...
        if workers:
            ...
            if worker_module == "res.partner":
                ...
                worker_domain = []
        return {"domain": {"worker_id": worker_domain}}
    ...
    def on_barcode_scanned(self, barcode):
        ...
        if self.production_id and self.workorder_id and self.worker_id:
            if (
                production != self.production_id
                or self.workorder_id != workorder
                or self.worker_id != worker
                or barcode == "#save"
            ):
    ...
    def record(self):  # , workorder, worker, qty_producing):
        ...
        workorder.write({"qty_producing": workorder.qty_producing, "qty_produced": workorder.qty_produced})
    ...
    def do_record(self):
        ...
        if self.production_id and self.workorder_id and self.worker_id:
            next = (
                self.record()
            )  # workorder=self.workorder_id, worker=self.worker_id, qty_producing=self.qty_producing)
