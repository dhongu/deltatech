...
class MergeProductAutomatic(models.TransientModel):
    ...
    def default_get(self, fields_list):
        res = super(MergeProductAutomatic, self).default_get(fields_list)
    ...
    def _update_reference_fields(self, src_products, dst_product):
        ...
        def update_records(model, src, field_model="model", field_id="res_id"):
            obj_model = self.env[model] if model in self.env else None
            if obj_model is None:
                return
            model_records = obj_model.sudo().search([(field_model, "=", "product.product"), (field_id, "=", src.id)])
            try:
                with mute_logger("odoo.sql_db"), self._cr.savepoint():
                    return model_records.sudo().write({field_id: dst_product.id})
            except psycopg2.Error:
                # updating fails, most likely due to a violated unique constraint
                # keeping record with nonexistent product_id is useless, better delete it
                return model_records.sudo().unlink()
    ...
    def _merge(self, product_ids, dst_product=None):
        ...
        if len(product_ids) > 3:
            raise UserError(
                _(
                    "For safety reasons, you cannot merge more than 3 product together."
        ...
        if (
            ...
            raise UserError(
                _(
                    """
Only the destination product may be linked to existing Journal Items.
"Please ask the Administrator if you need to merge several contacts linked to existing Journal Items.
                    """
                )
            )
    ...
    @api.model
    def _generate_query(self, table_fields, maximum_group=100):
        """ Build the SQL query on product.product table to group them according to given criteria
            :param table_fields : list of column names to group by the products
        ...
        # build the query
        text = ["SELECT min(id), array_agg(id)", "FROM product_product"]
    ...
    def _action_next_screen(self):
        ...
        if self.line_ids:
            ...
            values.update({"current_line_id": False, "product_ids": [], "state": "finished"})
