...
class service_consumption(models.Model):
    ...
    invoiced_qty = fields.Float(
        string="Invoiced Quantity", digits=dp.get_precision("Product Unit of Measure"), readonly=True, default=0.0
    )

    price_unit = fields.Float(
        string="Unit Price",
        required=True,
        digits=dp.get_precision("Service Price"),
        readonly=True,
        states={"draft": [("readonly", False)]},
        default=1,
    )
    ...
    invoice_id = fields.Many2one(
        "account.invoice", string="Invoice Reference", ondelete="set default", readonly=True, copy=False, index=True
    )

    uom_id = fields.Many2one(
        "product.uom", string="Unit of Measure", related="agreement_line_id.uom_id", readonly=True, copy=False
    )
    ...
    _sql_constraints = [
        (
            ...
            "unique(period_id,agreement_line_id)",
