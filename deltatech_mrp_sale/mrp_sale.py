...
class sale_order(models.Model):
    ...
    specification = fields.Boolean(string='Specification', default=True, readonly=True,
                                   states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, )
    # articole de deviz
    article_ids = fields.One2many('sale.mrp.article', 'order_id', string="Articles", copy=True, readonly=True,
                                  states={'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    ...
    qty_primary = fields.Float(string='Primary Quantity', readonly=True,
    ...
    # extrase de resurse
    resource_ids = fields.One2many('sale.mrp.resource', 'order_id', string="Resources", copy=False, readonly=True,
                                   states={'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    resource_item_ids = fields.One2many('sale.mrp.resource.item', inverse_name='order_id', string="All Products",
                                        copy=True)

    attributes = fields.One2many(comodel_name='sale.mrp.order.attribute', inverse_name='order_id',
                                 string='Order attributes', copy=True, readonly=True,
                                 states={'draft': [('readonly', False)], 'sent': [('readonly', False)]})
...
class sale_mrp_article(models.Model):
    ...
    def explode_bom(self):
        ...
        if self.bom_id:
            ...
            factor = self.env['product.uom']._compute_qty(self.product_uom.id, self.product_uom_qty,
        ...
        for item in items:
            ...
            if product:  # and item['product_qty'] <> 0:
                ...
                if self.order_id.pricelist_id:
                    ...
                    price_unit = self.order_id.pricelist_id.price_get(item['product_id'], item['product_qty'] or 1.0,
                                                                      self.order_id.partner_id.id)[price_list_id]
                    price_unit = self.env['product.uom']._compute_price(product.uom_id.id, price_unit,
                                                                        item['product_uom'])
                else:
                    price_unit = 0

                from_currency = self.env.user.company_id.currency_id.with_context(date=self.order_id.date_order)
                purchase_price = from_currency.compute(product.standard_price or product.product_tmpl_id.standard_price,
...
class sale_mrp_resource(models.Model):
    ...
    categ_id = fields.Many2one('product.category', related='product_id.categ_id',
                               store=True)  # este nevoie ? mai bine se aduce informatia direct in raport ?

    purchase_price = fields.Float(string='Purchase Price')

    bom_id = fields.Many2one('mrp.bom', string='BOM', compute='_compute_get_bom')

    resource_item_ids = fields.One2many('sale.mrp.resource.item', inverse_name='resource_id', string="Products",
                                        copy=True)
    ...
    def onchange_product_id(self):
        if self.product_id and self.article_id.order_id and self.article_id.order_id.pricelist_id:
            ...
            self.purchase_price = from_currency.compute(
                self.product_id.standard_price or self.product_id.product_tmpl_id.standard_price,
    ...
    def explode_product(self, product_id, product_uom_qty, product_uom_id):
        ...
        if not bom:
            ...
            bom_items, work = bom.with_context(production=self.article_id)._bom_explode(product=product_id,
                                                                                        factor=factor)
...
class sale_mrp_resource_item(models.Model):
    ...
    order_id = fields.Many2one('sale.order', related='article_id.order_id', store=True, string='Order', copy=False)
    article_id = fields.Many2one('sale.mrp.article', related='resource_id.article_id', store=True, string='Article',
