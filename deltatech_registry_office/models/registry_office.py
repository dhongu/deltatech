...
class RegistryOfficDoc(models.Model):
    ...
    category = fields.Selection(
        [("E", "Entry document"), ("O", "Output document"), ("I", "Internal document")],
        readonly=True,
    ...
    sender_id = fields.Many2one(
        "res.partner", string="Sender", required=True, readonly=True, states={"new": [("readonly", False)]}, index=True
    )  # Provenienta
    ...
    name = fields.Char(
        "Number", index=True, readonly=True, states={"new": [("readonly", False)]}, required=True, default="/"
    )
    ...
    state_id = fields.Many2one(
        "res.country.state", related="sender_id.state_id", readonly=True, states={"new": [("readonly", False)]}
    )
    zip = fields.Char(related="sender_id.zip", readonly=True, states={"new": [("readonly", False)]})
    country_id = fields.Many2one(
        "res.country", related="sender_id.country_id", readonly=True, states={"new": [("readonly", False)]}
    )
    ...
    type_id = fields.Many2one(
        "registry.office.doc_type", readonly=True, states={"new": [("readonly", False)]}, required=True,
    )  # Tip de act
    ...
    related_doc_id = fields.Many2one(
        "registry.office.doc", readonly=True, states={"new": [("readonly", False)]},
    )  # o	Conex cu numarul

    doc_ids = fields.One2many("registry.office.doc", "related_doc_id", readonly=True)

    solution_deadline = fields.Integer(
        default=30, readonly=True, states={"new": [("readonly", False)]},
    )  # Termen de rezolvare
    ...
    state = fields.Selection(
        registry_office_common.selection_state, default="new", string="Status", track_visibility="always"
    )
    ...
    def action_start(self):
        ...
        self.write({"state": "progress", "user_id": self.env.user.id, "date_start": fields.Datetime.now()})
        ...
        if self.user_id != self.env.user:
            ...
            if msg and not self.env.context.get("no_message", False):
                ...
                message = (
                    ...
                    .with_context({"default_starred": True})
                    .create(
                        {
