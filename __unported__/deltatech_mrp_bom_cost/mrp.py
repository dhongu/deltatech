...
class mrp_production(models.Model):
    ...
    def action_produce(self, cr, uid, production_id, production_qty, production_mode, wiz=False, context=None):
        ...
        if production.product_id.cost_method == 'fifo' and production.product_id.standard_price != production.calculate_price:
            self.pool.get('product.product').write(cr, uid, [production.product_id.id],
                                                   {'standard_price': production.calculate_price})

        res = super(mrp_production, self).action_produce(cr, uid, production_id, production_qty, production_mode, wiz,
                                                         context)
...
class mrp_bom(models.Model):
    ...
    value_overhead = fields.Float(string='Value Overhead', help="For Value Overhead percent enter % ratio between 0-1.",
                                  default='0.0')
    calculate_price = fields.Float(compute='_calculate_price', digits=dp.get_precision('Account'), store=False,
                                   string='Calculate Price')
    standard_price = fields.Float(compute='_calculate_standard_price', digits=dp.get_precision('Product Price'),
                                  store=False, string="Cost Price")
