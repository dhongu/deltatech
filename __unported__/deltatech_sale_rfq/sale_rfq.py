...
class sale_rfq(models.Model):
    ...
    order_id = fields.Many2one('sale.order', string="Quotation", readonly=True, states={
                               'draft': [('readonly', False)], 'in_progress': [('readonly', False)]},)
    lead_id = fields.Many2one('crm.lead', string='Opportunity', required=True,
                              readonly=True, states={'draft': [('readonly', False)]},)
    partner_id = fields.Many2one('res.partner', string='Customer', related="lead_id.partner_id", readonly=True, states={
                                 'draft': [('readonly', False)]}, store=True)    # Clientul
    ...
    request_date = fields.Date('Request Date', default=lambda * a: fields.Date.today(),
                               readonly=True, states={'draft': [('readonly', False)]})
    ...
    def _compute_doc_count(self):
        sale_rfq_docs = self.env['ir.attachment'].search(
            [('res_model', '=', 'sale.rfq'), ('res_id', '=', self.id)], count=True)
        leads_docs = self.env['ir.attachment'].search(
            [('res_model', '=', 'crm.lead'), ('res_id', '=', self.lead_id.id)], count=True)
    ...
    def new_quotation(self):
        ...
        message = _("Opportunity %s has been <b>converted</b> to the quotation <em>%s</em>.") % (
            self.get_link(self.lead_id), self.get_link(order_id))
