...
class stock_transfer_details_items(models.TransientModel):
    ...
    weight = fields.Float('Gross Weight', digits=dp.get_precision('Stock Weight'),
                          help="The gross weight in Kg.", related="result_package_id.weight")
        ...
        for packop in self:
            if not packop.result_package_id:
                if not newpack:
                    ...
                    newpack = self.pool['stock.quant.package'].create(
                        self._cr, self._uid, {'location_id': packop.destinationloc_id.id if packop.destinationloc_id else False}, self._context)
