...
class sale_order(models.Model):
    ...
    specification = fields.Boolean(string='Specification', default=True, readonly=True, states={
                                   'draft': [('readonly', False)], 'sent': [('readonly', False)]},)
    # articole de deviz
    article_ids = fields.One2many('sale.mrp.article', 'order_id', string="Articles", copy=True, readonly=True, states={
                                  'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    ...
    # extrase de resurse
    resource_ids = fields.One2many('sale.mrp.resource', 'order_id', string="Resources", copy=False, readonly=True, states={
                                   'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    resource_item_ids = fields.One2many(
        'sale.mrp.resource.item', inverse_name='order_id', string="All Products", copy=True)
...
class sale_mrp_article(models.Model):
    ...
    product_uom_qty = fields.Float(string='Product Quantity', required=True,
    ...
    def onchange_product_template(self):
        ...
        if not self.product_template.attribute_line_ids:
            self.product_id = (self.product_template.product_variant_ids
                               and self.product_template.product_variant_ids[0])
        else:
    ...
    def explode_bom(self):
        ...
        if self.bom_id:
            ...
            factor = self.env['uom.uom']._compute_qty(
                self.product_uom.id, self.product_uom_qty, self.bom_id.product_uom.id)
        ...
        for item in items:
            ...
            if product:    # and item['product_qty'] <> 0:
                ...
                if self.order_id.pricelist_id:
                    ...
                    price_unit = self.order_id.pricelist_id.price_get(
                        item['product_id'], item['product_qty'] or 1.0, self.order_id.partner_id.id)[price_list_id]
                ...
                purchase_price = from_currency.compute(
                    product.standard_price or product.product_tmpl_id.standard_price, self.order_id.pricelist_id.currency_id)
...
class sale_mrp_resource(models.Model):
    ...
    price_unit = fields.Float(string='Unit Price', digits=dp.get_precision('Product Price'))
    amount = fields.Float(string='Amount', digits=dp.get_precision('Account'), compute='_compute_amount', store=True)
    # este nevoie ? mai bine se aduce informatia direct in raport ?
    ...
    resource_item_ids = fields.One2many(
        'sale.mrp.resource.item', inverse_name='resource_id', string="Products", copy=True)
    ...
    def onchange_purchase_price(self):
        if self.purchase_price and self.product_id and self.order_id and self.order_id.pricelist_id:
            ...
            price = order_id.pricelist_id.price_get(self.product_id.id, self.product_uom_qty or 1.0, order_id.partner_id.id)[
    ...
    def onchange_product_id(self):
        if self.product_id and self.article_id.order_id and self.article_id.order_id.pricelist_id:
            ...
            price = order_id.pricelist_id.with_context(production=self.article_id).price_get(
                self.product_id.id, self.product_uom_qty or 1.0, order_id.partner_id.id)[order_id.pricelist_id.id]

            # nu cred ca e cazul ca sa mai schimb si unitatea de masura
            price = self.env['uom.uom']._compute_price(self.product_id.uom_id.id, price, self.product_uom.id)
            self.price_unit = price
            ...
            self.purchase_price = from_currency.compute(
                self.product_id.standard_price or self.product_id.product_tmpl_id.standard_price, order_id.pricelist_id.currency_id)
    ...
    def explode_product(self, product_id, product_uom_qty, product_uom_id):
        ...
        if not bom:
            ...
            bom_items, work = bom.with_context(production=self.article_id)._bom_explode(
                product=product_id, factor=factor)
...
class sale_mrp_resource_item(models.Model):
    ...
    order_id = fields.Many2one('sale.order', related='article_id.order_id', store=True, string='Order', copy=False)
    article_id = fields.Many2one('sale.mrp.article', related='resource_id.article_id',
                                 store=True, string='Article', copy=False, ondelete='cascade')
