...
class CrmClaimStage(models.Model):
    ...
    team_ids = fields.Many2many(
        ...
        " When set, this limitate the current stage to the selected sales teams.",
    )
...
class CrmClaim(models.Model):
    ...
    categ_id = fields.Many2one(
        "crm.case.categ", "Category", domain="[('team_id','=',team_id),   ('object_id.model', '=', 'crm.claim')]"
    )
    ...
    user_id = fields.Many2one(
        "res.users", "Responsible", track_visibility="always", default=lambda self: self.env.user.id
    )
    ...
    company_id = fields.Many2one(
        "res.company", "Company", default=lambda self: self.env["res.company"]._company_default_get("crm.case")
    )
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        return super(CrmClaim, self).message_new(msg_dict, custom_values=defaults)
