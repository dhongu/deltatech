...
- Phase on Purchase Orders
  ...
  - Helper field `phase_ids` (Many2many with compute/inverse) to enable flexible
    UI widgets and search domains that expect a multi-value relation, while still
    storing a single effective phase via `phase_id`.
...
- Models/fields
  - `purchase.order`:
    ...
    - `phase_ids = fields.Many2many("purchase.order.phase", compute=..., inverse=...)`
      The compute mirrors `phase_id` to a single-item M2M; the inverse writes back the
      first selected phase into `phase_id`.
