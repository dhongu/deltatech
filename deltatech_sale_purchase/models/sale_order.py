        ...
        for order in self:
            for order_line in order.order_line:
                for move in order_line.move_ids:
                    if move.created_purchase_line_ids:
                        if move.created_purchase_line_ids.order_id.state == "draft":
                            move.created_purchase_line_ids.unlink()
        ...
        for (parent, responsible), rendering_context in filtered_documents.items():
            if parent._name == "purchase.order":
                ...
                for order_line, diff in order_exceptions.values():
                    for move in order_line.move_ids:
                        purchase_line = move.created_purchase_line_ids
                        if purchase_line:
