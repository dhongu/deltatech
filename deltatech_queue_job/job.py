...
class DelayableRecordset(object):
    ...
    def __getattr__(self, name):
        if name in self.recordset:
            raise AttributeError("only methods can be delayed ({} called on {})".format(name, self.recordset))
    ...
    def __str__(self):
        return "DelayableRecordset({}{})".format(self.recordset._name, getattr(self.recordset, "_ids", ""))
...
class Job(object):
    ...
    def load(cls, env, job_uuid):
        ...
        if not stored:
            raise NoSuchJobError("Job %s does no longer exist in the storage." % job_uuid)
    ...
    def perform(self):
        ...
        try:
            ...
            if err.ignore_retry:
                ...
                new_exc = FailedJobError("Max. retries (%d) reached: %s" % (self.max_retries, value or type_))
    ...
    def store(self):
        ...
        if db_record:
            ...
            _logger.info("Trigger ir_cron_queue_job ")
