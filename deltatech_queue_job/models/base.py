...
class Base(models.AbstractModel):
    ...
    def with_delay(
        ...
        if os.getenv("TEST_QUEUE_JOB_NO_DELAY"):
            _logger.warning("`TEST_QUEUE_JOB_NO_DELAY` env var found. NO JOB scheduled.")
            return self
        if self.env.context.get("test_queue_job_no_delay"):
            _logger.warning("`test_queue_job_no_delay` ctx key found. NO JOB scheduled.")
            return self
    ...
    def _patch_job_auto_delay(self, method_name, context_key=None):
        ...
        def auto_delay_wrapper(self, *args, **kwargs):
            ...
            if (
                ...
                job_options_method = getattr(self, "{}_job_options".format(method_name), None)
                job_options = {}
