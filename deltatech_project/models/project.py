...
class project_project(models.Model):
    ...
    categ_id = fields.Many2one(
        "project.task.type", string="Category"
    )  # todo: de verificat unde e definit project.category
    project_parent_id = fields.Many2one(
        "project.project", string="Parent Project"
...
class project_task(models.Model):
    ...
    project_progress = fields.Float(
        string="Progress Project"
    )  # progresul cu care se afecteaza proiectul daca se indepineste obiectivul
    recurrence = fields.Boolean(string="Recurrence")
    current = fields.Boolean(
        string="Current Task", default=True, copy=False, compute="_get_current", store=True
    )  # se va actualiza cu CRON
