...
class TestNegative(TransactionCase):
    ...
    def test_allow_positive(self):
        self.env['stock.quant']._update_available_quantity(product_id=self.product, location_id=self.stock_location, quantity=10.0)
    ...
    def test_prevent_negative(self):
        self.env['stock.quant']._update_available_quantity(product_id=self.product, location_id=self.stock_location, quantity=5.0)
        move = self.env['stock.move'].create({
            'name': 'test_negative',
            'location_id': self.stock_location.id,
            'location_dest_id': self.pack_location.id,
    ...
    def test_allow_positive_lot(self):
        self.env['stock.quant']._update_available_quantity(product_id=self.product_lot, location_id=self.stock_location, quantity=10.0, lot_id=self.lot)
        move = self.env['stock.move'].create({
            'name': 'test_negative',
            'location_id': self.stock_location.id,
            'location_dest_id': self.pack_location.id,
    ...
    def test_prevent_negative_lot(self):
        self.env['stock.quant']._update_available_quantity(product_id=self.product_lot, location_id=self.stock_location, quantity=5.0, lot_id=self.lot)
        move = self.env['stock.move'].create({
            'name': 'test_negative',
            'location_id': self.stock_location.id,
            'location_dest_id': self.pack_location.id,
    ...
    def test_allow_positive_package(self):
        ...
        self.env['stock.quant']._update_available_quantity(product_id=self.product, location_id=self.stock_location, quantity=10.0, package_id=package)
        move = self.env['stock.move'].create({
            'name': 'test_negative',
            'location_id': self.stock_location.id,
            'location_dest_id': self.pack_location.id,
