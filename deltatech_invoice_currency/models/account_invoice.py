...
class account_invoice(models.Model):
    ...
    base_rate = fields.Float(
        string="Rate", digits=(12, 4), readonly=True, default=0.0, store=False, compute="_compute_base_rate"
    )
    ...
    def get_currency_rate(self):
        ...
        for invoice in self:
            ...
            if invoice.currency_id != invoice.price_currency_id:
                ...
                advance_payment_product_id = (
                    self.env["ir.config_parameter"].sudo().get_param("sale.default_deposit_product_id", False)
                )
        ...
        if self.product_id:
            ...
            if invoice.type == "out_invoice" and partner.property_product_pricelist:
                ...
                price_unit = pricelist.get_product_price(
                    product, qty, partner, date=invoice.date or fields.Date.today()
                )
