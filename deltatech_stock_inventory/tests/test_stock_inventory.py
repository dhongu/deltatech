...
class TestStockInventory(TransactionCase):
    def setUp(self):
        ...
        self.inventory_user = self.env['res.users'].with_context(
    ...
    def test_stock_inventory(self):
        ...
        inventory = self.env["stock.inventory"].with_user(self.inventory_user).create(
            {
    ...
    def test_new_inventory(self):
        inventory = self.env["stock.inventory"].with_user(self.inventory_user).create({"location_ids": [(6, 0, self.stock_location.ids)]})
        inventory.with_user(self.inventory_user).action_start()

    def test_stock_change_product_qty(self):
        wizard = Form(self.env["stock.change.product.qty"].with_user(self.inventory_user).with_context(active_ids=self.product_a.ids))
