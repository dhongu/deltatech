...
class StockProfitReport(models.Model):
    ...
    def _sub_select(self):
        ...
        sum(sq.quantity * pt.list_price) as sale_value,
        sq.customer_id,
        sq.supplier_id,
        pt.manufacturer,
        SUM(CASE
    ...
    def _from(self):
        from_str = """
         stock_quant as sq
            ...
            left join (
                    select sum(quantity) as qty_available , stock_quant.product_id, stock_quant.location_id,
                        ...
                        from stock_quant
                         ...
                         where stock_location.usage =  'internal'
                          group by stock_quant.product_id, stock_quant.location_id,   supplier_id, customer_id
                           ...
                           (available.supplier_id = sq.supplier_id or
                           (available.supplier_id is null and sq.supplier_id is null )) and
                           (available.customer_id = sq.customer_id or
                           (available.customer_id is null and sq.customer_id is null ))
        """
        return from_str
