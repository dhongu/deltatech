...
class ProcurementComputeProducts(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if "group_id" in defaults:
            ...
            for poline in polines:
                qty[poline.product_id.id] -= poline.product_uom._compute_quantity(
                    poline.product_qty, poline.product_id.uom_id, round=False
                )
        ...
        for product in products:
            ...
            if float_compare(qty[product.id], 0, precision_rounding=product.uom_id.rounding) > 0:
                defaults["item_ids"].append(
                    (0, 0, {"product_id": product.id, "qty": qty[product.id], "uom_id": product.uom_id.id})
                )
    ...
    def _procure_calculation_products(self):
        with api.Environment.manage():
            ...
            if self.group_id:
                ...
                ProcurementGroup.with_context(product_ids=products.ids).run_scheduler(
                    use_new_cursor=self._cr.dbname, company_id=self.env.user.company_id
                )
    ...
    def individual_procurement(self):
        ...
        if productions and active_model == "mrp.production":
            ...
            new_wizard = self.with_context(new_context).create(
                {
