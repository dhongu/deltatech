...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        ...
        if not getattr(threading.currentThread(), 'testing', False) and not self.env.registry.in_test_mode():
            sales = self.filtered(lambda p: p.company_id.sale_order_sms_validation  and (p.partner_id.mobile or p.partner_id.phone))
            for sale in sales:
                # Sudo as the user has not always the right to read this sms template.
                template = sale.company_id.sudo().sale_sms_confirmation_template_id
