...
class SaleOrder(models.Model):
    ...
    def _send_order_confirmation_mail(self):
        if not getattr(threading.current_thread(), "testing", False):
            sales = self.filtered(lambda o: o.company_id.sale_order_sms_post and o.partner_id.phone)
            for sale in sales:
    ...
    def action_confirm(self):
        ...
        if not getattr(threading.current_thread(), "testing", False):
            sales = self.filtered(lambda p: p.company_id.sale_order_sms_confirm and p.partner_id.phone)
            for sale in sales:
                # Sudo as the user has not always the right to read this sms template.
