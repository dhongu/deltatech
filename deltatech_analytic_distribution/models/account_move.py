            ...
            for line in self.invoice_line_ids:
                ...
                for key, value in analytic_distribution.items():
                    ...
                    try:
                        ...
                        raise ValidationError(
                            self.env._("Invalid value in analytic distribution: %s. It must be a number.") % value
                        ) from err

                # Check if the total percentage adds up to 100
                if total_percentage != 100.0:  # Allowing small floating-point errors
                    raise ValidationError(
                        self.env._("The total percentage in analytic distribution must add up to 100. Found: %s.")
                        % total_percentage
                    )
