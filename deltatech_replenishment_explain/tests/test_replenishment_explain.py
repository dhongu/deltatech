...
class TestReplenishmentExplainDiagram(TransactionCase):
    ...
    def test_coordinates_inside_track(self):
        g = self._geo()
        # everything stays within the track bounds
        for key in ("forecast_x", "min_x", "max_x", "gate_x", "lead_x", "window_x"):
            self.assertGreaterEqual(g[key], g["x0"], key)
            self.assertLessEqual(g[key], g["x1"], key)

    def test_to_order_gap_when_below_min(self):
        g = self._geo(below_min=True, order_forecast=2.0, target=10.0)
        # forecast (2) is below target (10) -> there is an amber "to order" gap
    ...
    def test_timeline_is_monotonic(self):
        g = self._geo(total_delay=7, visibility_days=3)
        self.assertLessEqual(g["today_x"], g["lead_x"])
        self.assertLessEqual(g["lead_x"], g["window_x"])

    def test_zero_horizon_does_not_divide_by_zero(self):
        # total_delay + visibility_days == 0 must not raise
        g = self._geo(total_delay=0, visibility_days=0)
        self.assertEqual(g["lead_x"], g["today_x"])

    def test_negative_forecast_clamped_to_left_edge(self):
        g = self._geo(order_forecast=-4.0)
